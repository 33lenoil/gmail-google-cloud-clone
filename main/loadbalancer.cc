...
// frontend ips
vector<string> frontendIPs = {"http://127.0.0.1:8001/", "http://127.0.0.1:8002/", "http://127.0.0.1:8003/"};
int counter = 0;
...
int main(int argc, char *argv[])
{
  ...
  if(vflag) fprintf(stderr, "[%d] LoadBalancer Node listining on port %d\n", listen_fd, portno);
  while(true) {
  ...
  // block SIGINT
    ...
    while(true) {
        ...
        if(req.valid) {
            ...
            if(uri == "/admin") {
              // only display admin console
              handler_map[uri]->handleRequest(req);
            } else {
              // send redirection message
              HttpResponse res(req);
              res.setStatus(302);
              string redirect = frontendIPs[counter];
              counter = (counter+1)%frontendIPs.size();
